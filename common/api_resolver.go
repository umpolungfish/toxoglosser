// common/api_resolver.go
// Contains shared API resolution functions to prevent import cycles
...
// Windows structures needed for API resolution and PE parsing
...
func GetModuleHandleByHash(moduleName string) (uintptr, error) {
	...
	currentEntry := (*LIST_ENTRY)(unsafe.Pointer(&loaderData.InMemoryOrderModuleList))
	nextEntry := (*LIST_ENTRY)(unsafe.Pointer(currentEntry.Flink))

	for nextEntry != (*LIST_ENTRY)(unsafe.Pointer(&loaderData.InMemoryOrderModuleList)) {
		// Get the LDR_DATA_TABLE_ENTRY
		// Calculate offset manually: InMemoryOrderLinks is the first field (offset 0)
		tableEntry := (*LDR_DATA_TABLE_ENTRY)(unsafe.Pointer(uintptr(unsafe.Pointer(nextEntry))))
